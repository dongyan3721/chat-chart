  ...
  methods: {
    ...
    initD3 () {
      ...
      const svg = d3.select('svg')
          ...
          .attr('viewBox', [-window.innerWidth/2, -3*window.innerHeight/4, window.innerWidth, 1.5*window.innerHeight])
              ...
              .join(
                  enter =>
                      enter.append('line')
                          ...
                          .attr('fill', '#fafafa')// 根据箭头标记的 id 号标记箭头
              )
      ...
      function addNodesAndLinks (node) {
        // 模拟接口返回节点和关系数据
        _this.$nextTick(() => {
          getKnowledgeFromOwnThinking({entity: node.name}).then(response=>{
            let resData = _this.adjustOwnThinkResponse(response)
            if (!resData) {
              ElMessage.warning("此标签暂时没有更多信息~")
              return
            }
            const edgeResult = []
    ...
    /**将数据样式调整成作者一样的样式*/
    adjustOwnThinkResponse(res) {
      if (Object.keys(res.data).length === 0) return null
      let edges = res.data.avp, ret = []
      edges.forEach(d => {
        ret.push({
          id: generateUUID(),
          named: d[0],
          ...
          direction: 0,
          relation_node: {
            id: calculateHash(d[1]),
            name: d[1],
            semantic_type: d[0],
            properties: res.data.desc,
